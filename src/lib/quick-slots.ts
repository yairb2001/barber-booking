/**
 * "Nearest available slot" scan shared by the public home carousel
...
export type QuickSlot = {
  ...
  date: string; dayLabel: string; time: string; timeMinutes: number;
  serviceId: string; serviceName: string; price: number; duration: number;
};

export async function computeQuickSlots(opts: {
  businessId: string;
  staffIdFilter?: string | null;
  /** Admin: every available barber, not only the quick pool. */
  allStaff?: boolean;
  preferredServiceId?: string | null;
  blockStatus?: { allBlocked: boolean; blockedStaffIds: Set<string> };
  /** Admin: ignore the customer-facing minimum lead time. */
  leadOverride?: number;
  ...
  const blockStatus = opts.blockStatus ?? { allBlocked: false, blockedStaffIds: new Set<string>() };
  ...
  for (const c of allCandidates) {
    ...
    if (picked.length >= perBarber) continue;
