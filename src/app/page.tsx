...
export default function HomePage() {
  ...
  const inviteFriend = async () => {
    ...
    const bizName = business?.name || "מספרה";
    const text = `קבעתי תור ב${bizName} 💈 ממליץ לך גם! קבע דרך הקישור:`;
    if (typeof navigator !== "undefined" && navigator.share) {
      try { await navigator.share({ title: bizName, text, url }); return; } catch { /* cancelled */ }
    }
  ...
  // Owner-configured pre-filled WhatsApp message — appended as ?text= so the
  // customer's chat opens with the message already typed, ready to send.
  const waHref = social.whatsapp
    ? social.whatsapp + (business?.whatsappPrefill ? `?text=${encodeURIComponent(business.whatsappPrefill)}` : "")
    : "";
    ...
    <div className="min-h-screen flex flex-col text-slate-900" dir="rtl" style={{ background: "var(--bg-page)" }}>
      ...
      <section className="relative flex flex-col" style={{ minHeight: "100svh" }}>
          ...
          <div className="flex gap-2">
            ...
            {social.whatsapp && (
              <div className="relative">
                ...
                {/* Nudge for customers who prefer booking over WhatsApp — a small
                    chat bubble under the icon; tapping it opens the same chat.
                    Owner can hide it from settings (default shown). */}
                {business?.whatsappBubbleEnabled !== false && (
                  <>
                    <a href={waHref} target="_blank" rel="noopener noreferrer"
                      className="absolute top-full left-0 mt-2 z-20 whitespace-nowrap active:scale-95 transition-transform"
                      style={{ animation: "waHintIn 0.45s ease-out 1.2s both, waHintBob 2.6s ease-in-out 1.7s infinite" }}>
                      <span className="relative block rounded-xl px-3 py-1.5 text-[11px] font-bold text-white shadow-lg"
                        style={{ background: "#25D366", boxShadow: "0 6px 20px rgba(37,211,102,0.45)" }}>
                        קבע תור דרך הוואטסאפ
                        <span className="absolute -top-1 left-4 h-2 w-2 rotate-45 rounded-[1px]" style={{ background: "#25D366" }} />
                      </span>
                    </a>
                    <style>{`
                      @keyframes waHintIn { from { opacity: 0; transform: translateY(-4px) scale(0.92); } to { opacity: 1; transform: translateY(0) scale(1); } }
                      @keyframes waHintBob { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(2px); } }
                    `}</style>
                  </>
                )}
              </div>
        ...
        <button
          ...
          className="relative z-10 mx-auto mb-4 mt-1 flex flex-col items-center gap-2.5 active:scale-95"
