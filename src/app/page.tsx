...
// === Works Gallery (swipeable, no auto-scroll) ===
        ...
        {works.map((work, i) => (
          <div key={i} className="flex-shrink-0 w-44 snap-start">
            <div className="aspect-[3/4] rounded-2xl overflow-hidden relative shadow-sm border border-neutral-100">
              ...
              />
              {/* Barber badge */}
              <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/70 to-transparent px-2.5 pb-2.5 pt-8">
                <div className="flex items-center gap-1.5">
                  {work.staffAvatar ? (
                    <img src={work.staffAvatar} alt="" className="w-5 h-5 rounded-full object-cover border border-white/40 flex-shrink-0" />
                  ) : (
                    <div className="w-5 h-5 rounded-full bg-white/30 flex items-center justify-center flex-shrink-0">
                      <span className="text-[9px] text-white font-medium">{work.staffName[0]}</span>
                    </div>
                  )}
  ...
  return (
    <div className="min-h-screen flex flex-col text-neutral-900 bg-white" dir="rtl">
      ...
      <div className="relative h-[52vh] min-h-[320px] max-h-[480px]">
        ...
        {/* Dark overlay — pointer-events-none כדי שהאייקונים למעלה יהיו לחיצים */}
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/30 to-black/40 pointer-events-none" />

        {/* Social icons — top, גדולים + לחיצים */}
        <div className="absolute top-5 inset-x-0 flex justify-between px-4 z-20">
          {/* שמאל: Waze + Facebook */}
          <div className="flex gap-2.5">
            {socialLinks.waze && (
              <a href={socialLinks.waze} target="_blank" rel="noopener noreferrer"
                className="w-11 h-11 rounded-full bg-white/85 backdrop-blur-md border border-white/50 flex items-center justify-center active:scale-95 transition shadow-md">
                <svg viewBox="0 0 24 24" className="w-5 h-5"><path fill="#33CCFF" d="M12 2C6.486 2 2 6.486 2 12c0 1.527.35 2.97.97 4.26L2 22l5.74-.97A9.953 9.953 0 0012 22c5.514 0 10-4.486 10-10S17.514 2 12 2zm-2 9a1 1 0 110-2 1 1 0 010 2zm4 0a1 1 0 110-2 1 1 0 010 2zm-5 3s1 2 3 2 3-2 3-2H9z" /></svg>
              </a>
            )}
            ...
            {business?.phone && (
              <a href={`tel:${business.phone}`}
                className="w-11 h-11 rounded-full bg-white/85 backdrop-blur-md border border-white/50 flex items-center justify-center active:scale-95 transition shadow-md">
                <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke="#374151" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 9.81a19.79 19.79 0 01-3.07-8.67A2 2 0 012 1h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.09 8.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z" />
                </svg>
              </a>
            )}
          </div>
          {/* ימין: Instagram + WhatsApp */}
          <div className="flex gap-2.5">
            {socialLinks.instagram && (
              <a href={socialLinks.instagram} target="_blank" rel="noopener noreferrer"
                className="w-11 h-11 rounded-full bg-white/85 backdrop-blur-md border border-white/50 flex items-center justify-center active:scale-95 transition shadow-md">
                <svg viewBox="0 0 24 24" className="w-5 h-5"><path fill="#E4405F" d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" /></svg>
              </a>
            )}
      ...
      {/* ===== עבודות הספרים ===== */}
      {!loading && <WorksGallery works={portfolioWorks} />}
