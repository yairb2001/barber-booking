...
async function main() {
  ...
  if (cmd === "status") {
    ...
    console.log(`${biz.name}: prompt ${cfg.systemPrompt?.length ?? 0} chars (${isCandidate ? "= CANDIDATE" : "live/other"}, sha ${sha(cfg.systemPrompt ?? "").slice(0, 12)}) · agentPromptV2=${settings.agentPromptV2 === true}`);
    return;
  }
  ...
  if (cmd === "promote") {
    ...
    await prisma.$transaction([
      ...
      prisma.business.update({ where: { id: biz.id }, data: { settings: JSON.stringify({ ...settings, agentPromptV2: true }) } }),
    ]);
    ...
    console.log(`promoted. backup: ${file} (${backup.systemPrompt?.length} chars) → live prompt ${DOMINANT_CANDIDATE_PROMPT.length} chars, agentPromptV2=true`);
    return;
  }
  ...
  if (cmd === "rollback") {
    ...
    const restoredSettings = { ...settings, agentPromptV2: false };
    await prisma.$transaction([
  ...
  throw new Error("usage: promote | rollback <file> | status");
}
