...
async function main() {
  ...
  const [c1, c2, c3, c4] = await Promise.all([mkCust("לוי לויאלי", "0521000001"), mkCust("חדש חדשי", "0521000002"), mkCust("מסרב סרבן", "0521000003"), mkCust("שקט שותק", "0521000004")]);
  ...
  await Promise.all([past(c1.id, yair.id, 30), past(c1.id, yair.id, 60), past(c1.id, yair.id, 90), past(c3.id, nitai.id, 20), past(c3.id, yair.id, 40), past(c4.id, yair.id, 25), past(c4.id, yair.id, 50)]);
  // the day being closed: 4 appointments with yair 13:00–15:00
  ...
  const [a1, a2, a3, a4] = await Promise.all([mk(c1.id, "13:00", "13:30"), mk(c2.id, "13:30", "14:00"), mk(c3.id, "14:00", "14:30"), mk(c4.id, "14:30", "15:00")]);

  console.log(`\n▶ sandbox ${biz.id} · closing ${day} 13:00–15:00 (dow ${dow})`);

  // ── 1) plan ──
  const plan = await planClosure({ businessId: biz.id, staffId: yair.id, date: day, fromTime: "13:00", toTime: "15:00" });
  console.log("1) PLAN");
  ok(plan.displaced.length === 4, `4 displaced (got ${plan.displaced.length})`);
  ...
  ok(plan.displaced.every(d => d.options.every(o => !(o.date === day && o.sameStaff && o.startTime >= "13:00" && o.startTime < "15:00"))), "no option inside the closed window");
  ...
  const res = await executeClosure({ businessId: biz.id, staffId: yair.id, date: day, fromTime: "13:00", toTime: "15:00" }, { reason: "בדיקה", excludeAppointmentIds: [a4.id], createdByStaffId: yair.id });
  ok(res.sent === 3 && res.excluded === 1 && res.failed === 0, `sent=3 excluded=1 failed=0 (got ${res.sent}/${res.excluded}/${res.failed})`);
  ...
  ok(props.length === 3, `3 proposals (excluded gets none) (got ${props.length})`);
  const holds = await prisma.slotHold.count({ where: { proposalId: { in: props.map(p => p.id) } } });
  ok(holds === 6, `6 slot holds (2 per proposal) (got ${holds})`);
  const ov = await prisma.staffScheduleOverride.findFirst({ where: { staffId: yair.id, date: dayObj } });
  ok(!!ov && ov.isWorking && JSON.stringify(JSON.parse(ov.slots || "[]")) === JSON.stringify([{ start: "10:00", end: "13:00" }, { start: "15:00", end: "18:00" }]), `override keeps 10–13 + 15–18 (got ${ov?.slots})`);
  const notices = await prisma.messageLog.findMany({ where: { businessId: biz.id, kind: "closure_notice" } });
  ok(notices.length === 3, `3 closure_notice logs (got ${notices.length})`);
  ...
  ok(await handleClosureReply(biz.id, c2.phone, `אפשר ב-${o2[1].startTime}`, today) === true, "c2 picked by time → consumed");
  ...
  ok(st[a1.id] === "rescheduled" && st[a2.id] === "rescheduled" && st[a3.id] === "agent" && st[a4.id] === "manual", `states: ${JSON.stringify(st)}`);
  ...
  const sw = await runClosureSweep(new Date());
  ...
  ok(!!summary && summary.customerPhone.endsWith("500000001") && summary.body.includes("2 נקבעו מחדש"), `summary went to the CLOSING BARBER only: "${summary?.body}"`);
