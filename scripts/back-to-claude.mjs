...
async function main() {
  ...
  const before = s.aiProvider;
  s.aiProvider = "anthropic"; // Claude. openaiModel/key stay for an easy flip back.
  await withRetry(() => prisma.business.update({ where: { id: BIZ_ID }, data: { settings: JSON.stringify(s) } }));
  console.log(`aiProvider: ${before} -> ${s.aiProvider}  (openaiModel kept as ${s.openaiModel})`);
}
