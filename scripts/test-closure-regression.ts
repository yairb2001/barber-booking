...
async function main() {
  ...
  const t0 = Date.now(); const startedAt = new Date();
  ...
  const wrote = await prisma.calendarClosure.count({ where: { businessId: biz!.id, createdAt: { gte: startedAt } } });
  console.log(`DOMINANT calendar_closures written during this run: ${wrote} (must be 0)`);
